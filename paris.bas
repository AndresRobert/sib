...
Sub generarArchivoRotulo()
    If Dir(ThisWorkbook.Path & "\bParis\eParis.xls", vbNormal) <> "" Then
        Kill ThisWorkbook.Path & "\bParis\eParis.xls"
    End If
    Columns("A:K").Copy
    Workbooks.Add
    Range("A1").PasteSpecial Paste:=xlPasteValues
    ActiveWorkbook.SaveAs Filename:=ThisWorkbook.Path & "\bParis\eParis.xls", FileFormat:=xlExcel8
    ActiveWindow.Close
End Sub
...
Sub ordenarLote()
    ...
    Range("I1").FormulaR1C1 = "COD. PARIS"
