...
Sub llenarDatos()
    Sheets("eOD").Select
    Range("E2").FormulaR1C1 = "=VLOOKUP(RC[-1],Maestras!C[-4]:C[-3],2,0)"
    Range("E2").AutoFill Destination:=Range("E2", Range("E2").End(xlDown))
    Columns(5).Copy
    Columns(5).PasteSpecial Paste:=xlValues
    Range("G1").FormulaR1C1 = "ATS"
    Range("G2").FormulaR1C1 = "=VLOOKUP(RC[-4],Maestras!C:C[1],2,0)"
    Range("G2").AutoFill Destination:=Range("G2", Range("F2").End(xlDown).Offset(0, 1))
    Columns(7).Copy
    Columns(7).PasteSpecial Paste:=xlValues
End Sub
...
Sub seleccionarBultos()
    ...
    While ActiveCell.Offset(Linea, -1) <> ""
        If ActiveCell.Offset(Linea, -5).Value <> _
            ActiveCell.Offset(Linea - 1, -5).Value Then
...
Sub numeroBulto()
    Sheets("eOD").Select
    Open ThisWorkbook.Path & "\bfoliosf.txt" For Input As #1
    ...
    Wend
    FolioBulto = FolioBulto + 1
    Open ThisWorkbook.Path & "\bfoliosf.txt" For Output As #1
        Print #1, FolioBulto
    Close #1
...
Sub crearNuevoDocumento()
    Sheets("EtiquetaBulto").Select
    If Dir(ThisWorkbook.Path & "\bFalabella\eFalabella.xls", vbNormal) <> "" Then
        Kill ThisWorkbook.Path & "\bFalabella\eFalabella.xls"
    End If
    Columns("A:D").Copy
    Workbooks.Add
    Range("A1").PasteSpecial Paste:=xlPasteValues
    ActiveWorkbook.SaveAs FileName:=ThisWorkbook.Path & "\bFalabella\eFalabella.xls", FileFormat:=xlExcel8
    ActiveWindow.Close
    Sheets("menu").Select
...
Sub llenarEPIR()
    ...
    Range("I1").FormulaR1C1 = InputBox("Ingrese el número de factura:")
    Range("J1").FormulaR1C1 = "=CONCATENATE(""1"",""|"",RC[-8],""|"",TEXT(RC[-7],""dd-mm-yyyy""),""|"",TEXT(RC[-6],""hh:mm""),""|"",RC[-5],""|0|0|0|"",RC[-1],""|||"")"
    Range("J2").FormulaR1C1 = "=CONCATENATE(""2"",""|"",RC[-8],""|"",RC[-7],""|"",RC[-6],""|"",RC[-5],""|BU|"",RC[-3])"
...
Sub crearArchivoEPIR()
    Sheets("ePIR").Select
    If Dir(ThisWorkbook.Path & "\bfalabella\ePIR-" & _
                                Sheets("ePIR").Range("B1").Value & "-" & _
                                Sheets("ePIR").Range("I1").Value & "-" & _
                                Sheets("Distrib").Range("G1").Value & ".txt", vbNormal) <> "" Then
        Kill ThisWorkbook.Path & "\bfalabella\ePIR-" & _
                                Sheets("ePIR").Range("B1").Value & "-" & _
                                Sheets("ePIR").Range("I1").Value & "-" & _
                                Sheets("Distrib").Range("G1").Value & ".txt"
    End If
    Open ThisWorkbook.Path & "\bfalabella\ePIR-" & _
                                Sheets("ePIR").Range("B1").Value & "-" & _
                                Sheets("ePIR").Range("I1").Value & "-" & _
